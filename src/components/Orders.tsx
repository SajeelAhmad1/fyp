      ...
      try {
        ...
        if (session?.user.id) {
          response = await fetch(
            `/api/orders/get-all-orders?userId=${session.user.id}`
          );
        } else if (email) {
          response = await fetch(
            `/api/orders/get-all-orders?guestEmail=${email}`
          );
        } else {
          return;
        }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-gray-100 p-4 flex border-b">
        <button
          ...
          className={`mr-4 py-2 px-4 rounded-t ${
            activeTab === "profile"
              ? "bg-white border-t border-l border-r font-bold"
              : ""
          }`}
        >
          Profile
          ...
          {isLoading ? (
            ...
            <div className="border rounded-lg overflow-hidden">
              <div className="p-6 text-center">
                ...
                <Link
                  href="/"
                  className="mt-4 inline-block bg-orange-500 text-white py-2 px-6 rounded"
                >
                  Start Shopping
                </Link>
              </div>
                ...
                <div key={order.id} className="border rounded-lg p-4">
                  ...
                  <div className="space-y-4">
                    {order.items.map((item) => (
                      <div key={item.id} className="flex flex-col md:flex-row md:items-center">
