...
export default function BillingDetails({
  ...
  useEffect(() => {
    if (formData.billingStreet) {
      ...
      const lines = formData.billingStreet.includes(',') 
        ? formData.billingStreet.split(',').map(line => line.trim())
  ...
  const handleStreetLineChange = (index: number, value: string) => {
    ...
    setFormData((prev: any) => ({
      ...prev,
      billingStreet: combinedStreetLines
    }));
  };
  ...
  const validateAddress = async () => {
    ...
    if (!formData.billingCity || !formData.billingState || !formData.billingPostalCode) {
    ...
    try {
      const response = await fetch('/api/validate-address', {
        ...
        body: JSON.stringify({
          address: {
            ...
            stateOrProvinceCode: formData.billingState,
      ...
      if (data.output?.resolvedAddresses?.[0]?.attributes?.Resolved) {
        ...
        setFormData((prev: any) => ({
          ...prev,
          billingStreet: streetLinesString,
          billingCity: resolved.city || formData.billingCity,
          billingState: resolved.stateOrProvinceCode || formData.billingState,
          billingPostalCode: (resolved.postalCodeToken?.value || formData.billingPostalCode)
        }));
  ...
  return (
    <div className="space-y-4 max-w-2xl mx-auto p-0 md:p-6">
      <h1 className="text-2xl font-semibold mb-6">billing Information</h1>
      ...
      <div>
        <div className="flex">
          ...
          <input
            ...
            className={`w-full p-2 border ${formErrors.billingPhone ? 'border-red-500' : 'border-gray-300'} rounded-r`}
          />
        </div>
      ...
      {streetLines.map((line, index) => (
        <div key={index} className="flex items-center space-x-2">
          <input
            ...
            className={`flex-1 p-2 border ${index === 0 && formErrors.billingStreet ? 'border-red-500' : 'border-gray-300'} rounded`}
            required={index === 0}
          />
          {index > 0 && (
            <button
      ...
      {formErrors.billingStreet && streetLines.length === 1 && streetLines[0].trim() === '' && (
      ...
      />
      {formErrors.billingPostalCode && <p className="text-red-500 text-xs mt-1">{postCodeError || 'Postal code is required'}</p>}
