...
export default function BillingDetails({
  ...
  const handleStreetLineChange = (index: number, value: string) => {
    ...
    const combinedStreetLines = newStreetLines
    ...
    try {
      ...
      if (data.output?.resolvedAddresses?.[0]?.attributes?.Resolved) {
        ...
        setFormData((prev: any) => ({
          ...
          billingPostalCode:
            resolved.postalCodeToken?.value || formData.billingPostalCode,
        }));
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex">
              ...
              <input
                ...
                className={`w-full p-2 border ${
                  formErrors.billingPhone ? "border-red-500" : "border-gray-300"
                } rounded-r`}
              />
            </div>
            ...
            <div key={index} className="flex items-center space-x-2">
              <input
                ...
                className={`flex-1 p-2 border ${
                  index === 0 && formErrors.billingStreet
                    ? "border-red-500"
                    : "border-gray-300"
                } rounded`}
                required={index === 0}
          ...
          {validationResult && (
            <div className="p-4 bg-gray-50 rounded-lg">
              ...
              {validationResult.resolvedAddresses?.map(
                (address: any, index: number) => (
                  ...
                  >
                    <h3 className="font-medium mb-2">
                      Resolved Address {index + 1}
                    </h3>
                    <p>
                      <strong>Street:</strong>{" "}
                      {address.streetLinesToken
                        ? address.streetLinesToken.join(", ")
                        : "N/A"}
                    </p>
                    <p>
                    ...
                    <p>
                      ...
                      {address.stateOrProvinceCode || "N/A"}
                    </p>
                    <p>
                      <strong>Postal Code:</strong>{" "}
                      {address.postalCodeToken?.value || "N/A"}
                    </p>
                    <p
                      className={`mt-2 ${
                        address.attributes?.Resolved
                          ? "text-green-600"
                          : "text-red-600"
                      }`}
                    >
                      <strong>Status:</strong>{" "}
                      {address.attributes?.Resolved ? "Valid" : "Invalid"}
                    </p>
                  </div>
                )
              )}
